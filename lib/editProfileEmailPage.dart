...
class EditProfileEmailPage extends StatefulWidget {
  ...
  const EditProfileEmailPage({Key? key}) : super(key: key);
...
class _EditProfileEmailPageState extends State<EditProfileEmailPage> {
  ...
  // to store the logged in token of the user
  ...
  Future editUserProfileEmail() async {

    setState(() {
      editProfileStatusInt = 4; //to show the loading circular icon
    });
    
    var url = "http://lenofiles.uchemcolin.xyz/api/users/update_profile_email";

    // the logged in user's token
    final token = loggedInUserToken;
    print("final token editUserProfileEmail: " + token);
    ...
    var response = await http.post(
      ...
      print("editUserProfileEmail() value");
      print(value);

      var response = value;

      print("response inside editUserProfileEmail()");
      ...
      if(response.body.isNotEmpty) {
        ...
        print("user details after successful email update:");
        print(userDetails);

        print("edit User Profile Email async function");
        print(formSubmission);
        print(response.body);
                        ...
                        ElevatedButton(
                          onPressed: () {
                            if (_formKey.currentState!.validate()) {
                              ...
                              editUserProfileEmail(); // update the user's email via the function
